...
- BFP-02 Part B live dataset construction plus the separate NEXT-01 historical runner: pinned stored splits, stage-specific shared full-catalog cohorts, deterministic random/positive-popularity/content baselines, source-and-result-bound validation sealing, and aggregate-only reports. NEXT-01 validation is complete; historical test remains sealed.
...
The historical adapter reports `ready` for final-test evaluation (1,708 eligible subjects), while the completed NEXT-01 train-to-validation benchmark evaluated a distinct 1,823-subject cohort. Its canonical aggregate results are NDCG@10 0.002261 random, 0.019733 positive-popularity, and 0.035004 content. The deployed ranker and live-customer evaluator are unchanged, and the historical test remains sealed.
