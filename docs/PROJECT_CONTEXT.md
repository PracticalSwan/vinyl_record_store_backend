...
- NEXT-04/NEXT-05 retain the historical Profile B closure evidence. The explicitly authorized 2026-08-29 maintenance supersedes only the current classroom/production environment with Profile C: three canonical synthetic showcase profiles, durable ratings/wishlist evidence, and the existing preference, behavior, aggregate-popularity, exact-feedback, and hybrid stages enabled. Committed defaults remain unchanged. See `DEMO_PERSONALIZATION_RUNBOOK.md`.
...
- Artwork delivery is source-aware. The proxy validates every Cover Art Archive/Internet Archive hop and varies Netlify caching by the requested `u` query. The 116 legacy files and 208 strict-v3 local files remain immutable source evidence. Customer presentation suppresses 46 duplicate display rows (2,259 visible) and adds 1,124 validated supplemental release-group artwork mappings; verified dataset art is local-first, supplemental art is proxy-only, and unresolved rows use the placeholder.
...
- Demo orders, real payments, broader production observability, ranking-flag source-default enablement, and live collaborative/matrix-factorization integration. GitHub-linked Netlify deployment is implemented; PERS-09 and administrator catalog APIs are implemented.
- PERS-00 through PERS-09 and NEXT-01 through NEXT-05 are complete and independently approved. PERS-04 through PERS-08 remain behind default-off source flags; production Profile C enables them by environment. The bounded biased-MF experiment was negative and remains offline-only; no historical evidence is a live hybrid quality claim.
