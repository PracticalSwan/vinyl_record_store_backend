...
Production deployment is GitHub-linked Netlify at `https://groovehaus-api.netlify.app/`, built from the sole `master` branch. The companion storefront is `https://groovehaus-store.netlify.app/` and proxies browser `/api/*` traffic same-origin. Production selects MongoDB/v3 Profile B by environment; committed source defaults remain unchanged.
