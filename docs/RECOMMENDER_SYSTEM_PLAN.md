...
## Professor-Compliant Item CF + Weighted Hybrid (2026-09-12)

The professor's later course requirement supersedes the earlier decision to keep all collaborative methods offline. `item-cf-v1` is now a default-off live Item-Based Collaborative Filtering component behind `PERS_ITEM_CF`. It takes positive account anchors (rating >= 4, wishlist, or positive cart state), reads positive historical `train` + `validation` ratings only, requires at least two co-positive listeners, applies binary-cosine similarity with shrinkage 5, and returns aggregate item evidence only. Unsupported or zero-score neighbors are not served.

When Item-CF is available with at least one other personalized component, `weighted-hybrid-v2` combines preference `0.405`, behavior `0.315`, Item-CF `0.10`, and popularity `0.18`, renormalizing over available components. The version is stamped `weighted-hybrid-v2` only when collaborative evidence actually participates. If CF is unavailable, the mathematically equivalent non-CF blend retains legacy `personalized-hybrid-v1`, so UI/course labels never claim CF when it did not run. Exact known-product exclusions and the final artist cap remain single-pass boundaries.
...
NEXT-04's Profile B remains historical closure evidence. The explicitly authorized 2026-08-29 maintenance selects MongoDB/v3 Profile C for the current classroom and production environment: preference, exact feedback, durable behavior, aggregate popularity, and `personalized-hybrid-v1` are enabled for three canonical synthetic showcase profiles. This is still not a source-default or quality-evaluation result. See `DEMO_PERSONALIZATION_RUNBOOK.md` for exact state, environment, checks, failure behavior, and rollback.
