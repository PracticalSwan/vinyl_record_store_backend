...
Stable fallback for the reviewed bundled catalog. `publicId` must be a canonical positive integer with no sign, whitespace, decimal, or leading zero and at most 10 digits. A mapped ID returns `307 Temporary Redirect` to `/artwork/<publicId>.<sha256-prefix>.jpg` with a five-minute revalidating cache policy; the content-addressed static JPEG returns `Cache-Control: public, max-age=31536000, immutable` and `X-Content-Type-Options: nosniff`. Malformed IDs return `400 ARTWORK_ID_INVALID`; valid but unmapped IDs return `404 ARTWORK_NOT_FOUND`. The generated manifest and release verifier require exact equality between the 116 catalog IDs, reviewed source mappings, manifest entries, and local JPEG files.
