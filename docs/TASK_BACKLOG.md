...
| B-027 | Run the sealed historical baseline validation benchmark. | done | NEXT-01 compares random, positive-popularity, and content on 1,823 validation-eligible subjects using train-only evidence, shared full-catalog candidates, aggregate-only output, and a source/result-bound seal. The final test remains unopened. |
| B-028 | Decide whether one offline collaborative experiment is justified. | done | NEXT-02 scored 10/12 and approved only dependency-free observed-rating `biased-matrix-factorization-v1`, offline-academic-only. Neighborhood CF and classical SVD were rejected; test authorization remains false until validation freezes a winner. |
| B-029 | Run the frozen offline biased-matrix-factorization experiment. | deferred | NEXT-03 is authorized for validation-only execution under the exact contract. No API/live integration is permitted and the historical test remains sealed. |
