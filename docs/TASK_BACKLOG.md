...
| B-021 | Add behavioral-signal personalization. | deferred | PERS-06 / BFP-13; current durable state + bounded opt-in passive content affinity; removals not negative taste. |
| B-022 | Add the popularity baseline and fallback ladder. | deferred | PERS-07 / BFP-14; active-dataset historical rating-count popularity; offline train-only baseline remains separate. |
| B-023 | Add hybrid recommendation orchestration. | deferred | PERS-08 / BFP-15; blend only when preference + behavior both exist, add popularity if available, keep lower modes pure, version `personalized-hybrid-v1`. |
