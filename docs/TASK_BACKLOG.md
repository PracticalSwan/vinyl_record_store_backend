...
| B-032 | Deploy the production backend. | done | GitHub-linked Netlify `groovehaus-api`; same-origin storefront proxy, Atlas/v3 environment, secure cookies, and Next runtime adapter verified. |
...
| B-035 | Enable role-aligned showcase hybrid personalization. | done | Profile C reconciles three synthetic personas with 9 ratings and 6 wishlist items, enables the implemented preference/behavior/popularity/hybrid stack by environment, protects durable state from broad test cleanup, excludes known items, and adds bounded candidate-read caching. Source defaults and historical evidence remain unchanged. |
| B-036 | Satisfy the professor's two-batch recommender requirement. | done | Added default-off `item-cf-v1` Item-Based CF over train+validation-only aggregate co-positive evidence and `weighted-hybrid-v2` with 10% CF weight, sparse fallback, truthful versioning, regression coverage, and professor-facing documentation. |
