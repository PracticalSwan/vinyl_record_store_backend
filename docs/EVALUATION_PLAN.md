...
| Current Profile C showcase contract | A bounded Chromium desktop flow signs in as all three protected showcase customers, requires `personalized-hybrid-v1`, checks Jazz/Rock/Soul dominant-result alignment, verifies rated/wishlisted records are excluded, captures one screenshot per persona, signs out, and runs the ownership-scoped Atlas teardown. This is functional demonstration evidence, not ranking-quality evidence. |
