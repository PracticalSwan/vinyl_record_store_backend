...
| Current Profile C professor-compliance contract | A bounded Chromium desktop flow signs in as all three protected showcase customers with `PERS_ITEM_CF=true`, requires `weighted-hybrid-v2`, verifies the professor-facing Item-Based CF + Weighted Hybrid label, checks role-aligned output and known-item exclusion, signs out, and runs ownership-scoped Atlas teardown. Repository tests separately prove collaborative reads are limited to historical `train` + `validation`. This is functional demonstration evidence, not ranking-quality evidence. |
...
The current environment enables the already-implemented deterministic preference, behavior, aggregate-popularity, exact-feedback, and hybrid stages for three canonical synthetic showcase profiles. The seed reconciles three ratings and two wishlist items per profile, the returned candidate list excludes those known records, and the cleanup policy preserves showcase and ordinary-customer durable state. The sealed historical final test was not rerun, so these checks establish behavior and explainability only; the live evaluator remains `insufficient-evidence`.

## Professor-Compliance Verification Addendum - 2026-09-12

The current release adds `item-cf-v1` and `weighted-hybrid-v2` without rereading the permanently consumed final holdout. The full backend suite completed 331 tests: 330 passed, 0 failed, and 1 intentional Windows symlink-permission skip. ESLint, the Next.js 16.3.5 production build, 116-file artwork verification, and `npm audit --audit-level=high` all passed; npm audit reported 0 vulnerabilities. The Atlas-backed Chromium showcase flow passed 1/1 for all three protected personas with the course-method label, role-aligned output, known-item exclusion, and zero test residue after teardown. These are behavior/security/release checks, not a new ranking-quality result; live quality remains `insufficient-evidence`.
