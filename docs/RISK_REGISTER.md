...
| BR-006 | Catalog metadata is incomplete, mismatched, or biased. | Medium | Validate imports, preserve source ownership, prefer exact official album-vinyl MusicBrainz matches, visually review every bundled cover, document six explicit identity exceptions, bind artwork to a release/release group, record provenance, hash-verify the exact local bundle, and retain placeholders. | controlled |
...
| BR-018 | External catalog or artwork services are unavailable, redirected, or rate-limited. | Medium | Use a meaningful User-Agent, one request per second for metadata, bounded timeouts, per-hop redirect allowlists, local cache, no broad scraping, warnings instead of data loss, no-write previews, and a verified committed fallback for all 116 bundled covers. | controlled |
...
| BR-029 | Raw external data, reviewer identity, or unlicensed content is committed or exposed. | High | Pin manifest/hashes, ignore raw/staging, HMAC reviewer keys, exclude review text/images/profiles, expose aggregate readiness only, and document the publisher's no-license boundary. | controlled |
