...
| BR-020 | Planned personalization is presented as real measured quality. | High | Honesty wording locked in PERS-00; every milestone keeps the `insufficient-evidence` status and forbids quality claims; synthetic fixtures labelled as demonstrations. | open |
| BR-021 | The arbitrary-user recommendation route leaks private profile data after personalization lands. | High | PERS-01 restricts the route so non-`demo-user` ids never read profile data; cross-user denial contract test gates PERS-03. | open |
