...
- PERS-00 through PERS-08 are implemented behind default-off flags: identity-safe subject descriptors, the session-owned `/api/recommendations/me` path, the server-internal profile/feedback domain, exact-item feedback, the `preference-profile-v1`, `behavior-profile-v1`, `popularity-v1`, and `personalized-hybrid-v1` branches. `content-demo-v1` remains the default and rollback path; PERS-09 remains deferred.
