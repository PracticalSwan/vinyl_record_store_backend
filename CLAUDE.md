...
- The reviewed 116-record local seed remains the no-database default and legacy rollback. Explicit `CATALOG_DATA_SOURCE=mongodb` follows immutable active `amazon-reviews-2023-cds-vinyl-5core-v2` in `datasetProducts`; v1 and the legacy catalog remain stored rollback targets.
...
- Corrected DATA-00 through DATA-15 v2 adds controlled metadata/year semantics, stable v1 IDs, immutable sealed rows and exact digests, strict MusicBrainz/CAA artwork with an exact local fallback set, 20,288 isolated historical ratings from 2,387 HMAC-pseudonymous subjects, transactional activation/v1 rollback, and aggregate-only readiness. Historical `ready` is not a model result; PERS-03 through PERS-09 remain deferred.
