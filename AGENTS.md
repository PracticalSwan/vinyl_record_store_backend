...
- PERS-00 through PERS-05 are implemented behind default-off flags: identity-safe subject descriptors, the session-owned `/api/recommendations/me` path, the server-internal profile/feedback domain, exact-item feedback, and the optional `preference-profile-v1` branch. `content-demo-v1` remains the default and rollback path; PERS-06 through PERS-09 remain deferred.
...
- Corrected DATA-00 through DATA-15 v3 adds controlled metadata/year semantics, authoritative release-group original years for 208 strict matches, stable v1 IDs, immutable sealed rows and exact digests, strict MusicBrainz/CAA artwork with an exact local fallback set, 20,288 isolated historical ratings from 2,387 HMAC-pseudonymous subjects, transactional activation with v2 as the immediate rollback release, and aggregate-only readiness. Historical `ready` is not a model result; PERS-03 through PERS-05 are implemented behind default-off flags, while PERS-06 through PERS-09 remain deferred.
...
- Artwork uses two backend-owned delivery paths. `GET /api/artwork?u=<approved URL>` is the bounded, disk-cached primary proxy and validates every Cover Art Archive/Internet Archive redirect hop. `GET /api/artwork/local/:publicId` resolves either the 116-record legacy manifest or the current v3 dataset manifest (208 files); separate stable evidence verifies the same fallback set for v2 rollback. Each content-addressed JPEG has generated provenance, size, dimensions, and SHA-256 evidence.
...
- Label user results as `demo-profile`, `cold-start`, `preference-profile`, or `anonymous-fallback` as returned. Exact feedback remains a server-owned exclusion; never imply measured recommendation quality or enablement when the new flags are off.
