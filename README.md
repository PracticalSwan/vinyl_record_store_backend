...
Next.js 16.3.5, React 19.2.4, Tailwind CSS 4, and Mongoose, tested with the Node test runner.
...
The current classroom setup is the explicit MongoDB/v3 **Profile C: Professor-Compliant Hybrid** environment, not the committed default. It enables real `item-cf-v1` Item-Based Collaborative Filtering plus `weighted-hybrid-v2` alongside saved preferences, durable behavior, aggregate popularity, and exact feedback. See [`docs/PROFESSOR_RECOMMENDER_COMPLIANCE.md`](docs/PROFESSOR_RECOMMENDER_COMPLIANCE.md) for formulas/selection rationale and [`docs/DEMO_PERSONALIZATION_RUNBOOK.md`](docs/DEMO_PERSONALIZATION_RUNBOOK.md) for the exact demo flow.
