...
test("product recommendations rank the same artist first", () => {
  const result = recommendForProduct(1, 6);
...
test("demo profile excludes its known records and labels its mode", () => {
  const result = recommendForUser("demo-user", 8);
...
test("unknown users receive an explicit cold-start list", () => {
  const result = recommendForUser("new-user", 5);
