...
The catalog defaults to the bundled seed even when MongoDB credentials are present. Set `CATALOG_DATA_SOURCE=mongodb` explicitly in environments that should use Atlas; missing configuration or connection failures return a safe unavailable error instead of silently falling back.
